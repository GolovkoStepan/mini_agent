...
module MiniAgent
  ...
  class StreamParser
    ...
    # Служебные токены шаблона, пробившиеся в текст размышлений. Это не то,
    # что модель думает, а разметка её собственного формата, и показывать её
    # ходом мысли неверно вдвойне: смысла в ней нет, а выглядит она как ответ
    # на вопрос «чем модель занята».
    #
    # Поймано живьём: на сорвавшейся задаче в спиннере стояло
    # «размышления: 23 знака │ <|mask_start|><think>» — по этой строке было
    # не отличить работающую модель от сломавшейся, а сломалась именно она.
    #
    # Убирается ТОЛЬКО из показа: в @reasoning и, значит, в журнал текст
    # уходит как пришёл. Утёкший токен — это диагноз, и терять его там, где
    # разбирают сорвавшийся ход, нельзя.
    SPECIAL_TOKEN = %r{<\|[^|>]*\|>|</?think>}
    ...
    #
    # Служебные токены вычищаются здесь, а не при накоплении: второй
    # накопитель того же текста разошёлся бы с первым при первой правке
    # (та же причина, по которой хвост — производная, а не своё поле).
    # Срез берётся с запасом и лишь потом чистится: режь мы ровно по лимиту,
    # разрез пришёлся бы на середину токена, и наружу вылез бы огрызок
    # вроде `k_start|>` — ровно то, от чего чистка и заводилась.
    def reasoning_tail
      window = @reasoning[-(TAIL_LIMIT * 2)..] || @reasoning
      visible = window.gsub(SPECIAL_TOKEN, "")
      visible[-TAIL_LIMIT..] || visible
    end
