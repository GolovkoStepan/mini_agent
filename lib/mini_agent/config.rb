...
module MiniAgent
  ...
  class Config
    ...
    DEFAULTS = {
      ...
      api_key: "lm-studio",
      # Выбрано по оценочным задачам, а не по размеру: на матрице из семи
      # задач qwen3.8-27b даёт 21/21 против 20/21 у qwen3.6-35b-a3b и тратит
      # на ход вдвое меньше размышлений (237 токенов против 560) — то есть
      # реже упирается в окно на длинной задаче. Живая работа по чужому
      # репозиторию это подтвердила: qwen3.6 на /init ушла в бесконечное
      # перечисление (13 вызовов инструментов в одном ответе, упор
      # в max_tokens), qwen3.8 ту же задачу довела за десять ходов.
      #
      # Медленнее по токенам в секунду (dense против MoE), но токенов ей нужно
      # втрое меньше, так что ход занимает столько же. Умолчание всё равно
      # верно лишь для того, у кого эта модель загружена, — потому и меняется
      # одной опцией.
