...
module MiniAgent
  ...
  class Agent
    ...
    #
    # Исход при этом :unfinished, а не :failed, и разница та же, что между
    # кодами 4 и 3: сервер ответил исправно, инструменты работали, модель
    # просто не сказала ничего — то есть повтор с теми же настройками
    # осмыслен, в отличие от обрыва по лимиту, где вторая попытка упрётся
    # в тот же лимит. Раньше исход не выставлялся вовсе, и агент выходил
    # с нулём, не сделав ничего: найдено оценочными задачами — на edit-method
    # модель выдала 1747 знаков размышлений, кончила их выводом «this should
    # work», не вызвала edit_file и отчиталась успехом.
