  ...
  module Messages
    ...
    module Tool
      ...
      OUTSIDE_CWD_CANCELLED = "Путь ведёт за пределы рабочего каталога, и запись ОТКЛОНЕНА. " \
                              "Ничего не изменилось и не создано. Если ты не имел в виду чужой " \
                              "каталог, значит путь набран с ошибкой: укажи его ОТНОСИТЕЛЬНО " \
                              "рабочего каталога (например lib/foo.rb) — он разрешается от него " \
                              "сам, и промахнуться мимо цели относительным путём нельзя. " \
                              "Не обходи этот отказ через bash: перенаправление по тому же " \
                              "пути промахнётся ровно так же. Не утверждай, что файл записан."
