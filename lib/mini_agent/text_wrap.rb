...
module MiniAgent
  ...
  #
  # На вход идут сегменты — пары «текст, список стилей», уже разобранные
  # Markdown. Список, а не один стиль: разметка вкладывается, и `код` внутри
  # **жирного** несёт оба.
  ...
  class TextWrap
    ...
    # Кусок без стилей. Живёт здесь, у того, кто сегменты принимает: форма
    # сегмента — его контракт, и Markdown#inline берёт пустой стиль отсюда,
    # а не заводит своё имя тому же значению. Пустой список, а не nil:
    # у сегмента стиль всегда список, потому что разметка вкладывается.
    NO_STYLE = [].freeze
    ...
    def gap_style(previous, following)
      previous && following && previous[1] == following[1] ? previous[1] : NO_STYLE
    end
    ...
    # Стилей у куска может быть несколько: разметка вкладывается, и код
    # внутри жирного несёт оба. Пустой список означает «красить нечем» —
    # проверять на истинность нельзя, пустой массив истинен.
    def paint_parts(parts)
      parts.map { |content, styles| styles.empty? ? content : @paint.call(content, *styles) }.join
    end
  end
end
