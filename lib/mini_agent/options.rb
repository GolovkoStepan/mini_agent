...
module MiniAgent
  ...
  class Options
    ...
    FLAGS = [
      ...
      # Аргумент необязателен: без него продолжается последняя сессия этого
      # каталога. Ключ попадает в values и с пустым значением — «продолжить»
      # и «не продолжать» различает наличие ключа, а не его значение.
      # Без аргумента, хотя продолжить хочется и названный файл. Необязательный
      # аргумент OptionParser забирает следующее слово argv, а следующим здесь
      # стоит задача: `mini_agent --resume почини тест` пытался бы продолжить
      # сессию из файла «почини». Файл называет --log, который и так задаёт,
      # в какой журнал писать, — второго способа назвать его быть не должно.
      [:resume, "--resume", Messages::OPT_RESUME],
      # Пара --session/--no-session, а не один отрицающий флаг: сохранение
      # включено по умолчанию, но выключить его можно и файлом настроек,
      # и тогда --session — единственный способ вернуть его на один запуск.
      [:session, "--[no-]session", Messages::OPT_NO_SESSION],
