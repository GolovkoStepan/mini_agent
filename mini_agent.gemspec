...
Gem::Specification.new do |spec|
  ...
  spec.files = IO.popen(%w[git ls-files -z], chdir: __dir__, err: IO::NULL) do |ls|
    ls.readlines("\x0", chomp: true).reject do |f|
      (f == gemspec) ||
        # evals/ — измерительный стенд разработчика: он гоняет агента
        # десятками прогонов и в установленном геме бесполезен.
        f.start_with?(*%w[bin/ test/ spec/ features/ evals/ .git appveyor Gemfile])
    end
  end
  spec.bindir = "exe"
